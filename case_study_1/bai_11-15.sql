...
WHERE
    customer_type_id = 1
        ...
        OR cus.address LIKE '%Quảng Ngãi') viên), ho_ten (khách hàng), so_dien_thoai (khách hàng), ten_dich_vu, so_luong_dich_vu_di_kem 
...
SELECT asv.asv_id,
SUM(dc.quantity) AS "time_used",
asv.asv_name
FROM additional_services asv
...
-- (được tính dựa trên việc count các ma_dich_vu_di_kem).
	...
	ON ser.service_type_id = st.type_id
	JOIN detailed_contracts dc
	ON con.contract_id = dc.contract_id
    ...
    ON ser.service_type_id = st.type_id
    GROUP BY dc.asv_id HAVING COUNT(dc.asv_id) = 1
    );
...
JOIN academic_level aca
ON emp.level_id = aca.level_id
JOIN department dep
ON emp.department_id = dep.department_id
WHERE (YEAR(con.start_date) BETWEEN 2020 AND 2021)
